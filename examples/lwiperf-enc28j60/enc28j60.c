...
#ifdef ENC28J60_SPI_CS
static inline void cs_select()
{
	asm volatile("nop \n nop \n nop");
	gpio_put(ENC28J60_SPI_CS, 0); // Active low
	asm volatile("nop \n nop \n nop");
}
...
{
	spi_write_blocking(ENC28J60_SPI_PORT, &data, 1);
}
...
uint8_t enc28j60ReadOp(uint8_t op, uint8_t address)
{
	...
	// read data
	uint8_t dst[1];
	spi_read_blocking(ENC28J60_SPI_PORT, 0, dst, 1);
...
void enc28j60ReadBuffer(uint16_t len, uint8_t *data)
{
	...
	#ifdef USE_SPI_DMA
	// 20230908 taylor
	channel_config_set_read_increment(&tx_dma_channel_config, false);
	channel_config_set_write_increment(&tx_dma_channel_config, false);
	dma_channel_configure(tx_dma_chan, &tx_dma_channel_config,
						  &spi_get_hw(ENC28J60_SPI_PORT)->dr, // write address
	...
	dma_channel_configure(
		...
		data,					   			// write address
		&spi_get_hw(ENC28J60_SPI_PORT)->dr, 		// read address
		len,
		false);
	...
	#else
	spi_read_blocking(ENC28J60_SPI_PORT, 0, data, len);
	#endif
...
void enc28j60WriteBuffer(uint16_t len, uint8_t *data)
{
	...
	// }
	spi_write_blocking(ENC28J60_SPI_PORT, data, len);
...
void enc28j60Init(uint8_t *macaddr)
{
	...
	enc28j60PhyWrite(PHLCON, 0x0050);
	#endif
	...
	channel_config_set_dreq(&rx_dma_channel_config, DREQ_SPI1_RX);
	...
	channel_config_set_dreq(&tx_dma_channel_config, DREQ_SPI1_TX);
